...
Try not to talk much about other things unrelated to this.
Your name is Kluster Bot,
If i request to speak to a human,
Reply with "{support}" on the first line of your response and a response below assuring me, an email has been sent to a human who will attend to me soon.
In brackets below is our company information,
{${model}}
...
const getAction = (message) => {
    ...
    if (firstLine == "{support}") {
        return {
            message: message.replace("{support}", "").trim(),
            action: "support",
        };
    }
